...
| :--- | :--- | :--- |
| 🐾 **OpenCatz Core Swarm** | Multichain Screening & Reasoning | **3-Layer Swarm Consensus Engine** (Quant & Liquidity, Catalyst & Sentiment, Security Audit) demanding a **≥ 80% Confidence Score** before any signal card is delivered. Eight specialist screening agents run 24/7:<br>• **Solana Memes** (`meme-solana`): Pump.fun graduated, Raydium, Meteora DLMM (GMGN + RugCheck).<br>• **Robinhood & EVM Memes** (`meme-robinhood`): Robinhood Chain & Base DEX tokens (GMGN + GoPlus).<br>• **Solana LP Velocity** (`lp-solana`): Meteora DLMM Concentrated Liquidity pools.<br>• **Robinhood LP Velocity** (`lp-robinhood`): Uniswap V3 concentrated pools via Krystal Cloud.<br>• **Whale & Perps Tracker** (`perps` / `call-whale-tracking`): Hyperliquid L1 institutional positioning & spot flow.<br>• **NFT Sniping** (`nft` / `call-nft-sniping`): OpenSea floor surge, rare trait sniping, and Catz NFT tracking.<br>• **Prediction Markets** (`prediction`): Polymarket odds mispricing & whale bets.<br>• **Smart CT Alpha** (`ct-alpha`): Twitter/X sentiment analysis & smart money calls. |
| 🎮 **Command Center** | Discord · Terminal · Telegram | **1. Discord**: `#opencatz-control-room` natural-language chat, `#opencatz-audit` on-demand 12-point audits, 9-Lives risk views, **23 slash commands** (including `/catz`), interactive dashboard, and 8 dedicated call channels.<br>**2. Terminal TUI** (`opencatz terminal`): Full interactive standalone console with live audits, multichain screening triggers, Catz NFT verification, and portfolio management.<br>**3. Telegram Bridge**: Real-time signal alert broadcast cards with quick inline action buttons. |
