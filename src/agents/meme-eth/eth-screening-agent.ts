...
export interface EthScreeningConfig {
  chains: SolChain[];
...
const DEFAULT_CONFIG: EthScreeningConfig = {
  chains: ['eth'],
...
export class EthScreeningAgent implements ScreeningAgent<EthSignal> {
  readonly domain = 'meme-eth';
  private gmgn: GMGNAdapter;
  private priceFeed = globalPriceFeedService;
  private strategyEngine: StrategyEngine;
  private config: EthScreeningConfig;
  private dedupeTokens = createDedupe();

  constructor(config?: Partial<EthScreeningConfig>) {
  ...
  public updateConfig(partial: Record<string, unknown>): { applied: Record<string, unknown>; rejected: string[] } {
    ...
    if (Array.isArray(partial.chains)) {
      const validChains = partial.chains.filter((c): c is SolChain => ['eth'].includes(c as any));
      if (validChains.length > 0) applied.chains = validChains;
      else rejected.push('chains: must be ["eth"]');
    }
    this.config = { ...this.config, ...applied };
    if (Object.keys(applied).length > 0) {
      console.log(`[ETH MEME AGENT] Config updated: ${JSON.stringify(applied)}`);
    }
    return { applied, rejected };
  }

  public getConfig(): EthScreeningConfig {
    return { ...this.config };
  }
    ...
    const allChainCandidates = await Promise.all(
      this.config.chains.map(async (chain) => {
        try {
          ...
          console.warn(`[ETH MEME AGENT] Failed candidate collection for ${chain}: ${err.message}`);
          return [];
        }
      })
    );
  ...
  public async collectSignalBoostMap(): Promise<SignalBoostMap> {
    try {
      ...
      console.warn(`[ETH MEME AGENT] Signal boost failed (skipped): ${err.message}`);
      return new Map();
    }
  }
  ...
  public async collectTrackAccumulation(): Promise<Map<string, TrackAccumulation>> {
    ...
    try {
      ...
      console.warn(`[ETH MEME AGENT] Smart money track failed for ETH: ${err.message}`);
      return new Map();
    }
  }
  ...
  public buildPayload(t: GMGNRawToken, confidence: number, thesis: string, trackLabel?: string): CallCardPayload {
    ...
    return {
      ...
      aiThesis: thesis,
      gmgnUrl: `https://gmgn.ai/eth/token/${t.address}`,
      dexScreenerUrl: `https://dexscreener.com/ethereum/${t.address}`,
      rugcheckUrl: `https://gopluslabs.io/token-security/1/${t.address}`,
  ...
  public async runScreeningPass(): Promise<AgentReport<EthSignal>[]> {
    console.log('[ETH MEME AGENT] Ethereum screening pass started (GMGN OpenAPI)...');
    const reports: AgentReport<EthSignal>[] = [];

    let ethPrice: number | null = null;
    try {
      ethPrice = await this.priceFeed.getPrice('ETH').catch(() => null);
    } catch (err: any) {
      console.warn(`[ETH MEME AGENT] Failed to fetch ETH price: ${err.message}`);
    }
    ...
    for (const t of candidates) {
      ...
      let strategyReason = '';
      try {
        const strat = this.strategyEngine.getActiveStrategy('meme-eth');
        if (strat?.evaluate) {
      ...
      } catch (err: any) { console.warn(`[ETH MEME AGENT] Strategy failed: ${err.message}`); }
      ...
      const signal: EthSignal = { token: t, signalType: det.type, confidence, reasons: det.reasons };
      reports.push({ passed: true, signal, reason: thesis, confidence, payload });
      console.log(`[ETH MEME AGENT] 🎯 [ETH] ${det.type} ${t.symbol} ${confidence}%`);
    }

    console.log(`[ETH MEME AGENT] Pass complete. ${reports.length} signals passed.`);
    return reports;
  }
}
